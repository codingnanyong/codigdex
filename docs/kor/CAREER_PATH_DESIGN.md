...
지역 정의는 `web/lib/phaser/worldMap/careerPaths.ts`에 있다. 현재 기술 지역 28곳은 모두 지도와 몬스터 아트가 준비되어 있고, 배틀·문제 콘텐츠는 **(예정)** 이다.

### 맵의 깊이와 이동

Path 화면과 플레이 지역은 같은 화면이 아니다. Path 맵은 공통 과정과 전직 관계를 보여 주는 진로도이고, 1차 직업을 선택하면 월드맵이 해당 직업의 **상세 지도**로 바뀐다. 상세 지도에서 기술 지역을 누르면 그 지역을 확대한 **기술 지역 화면**으로 들어가며, 여기서 안내 NPC가 수집지를 소개한다. 기술 지역에서 몬스터 배틀로 이어지는 연결은 전문 챕터 콘텐츠와 함께 추가한다 **(예정)**.

```text
Path 맵 → 직업별 상세 지도 → 기술 지역 → 배틀/포획 (예정)
```
...
기술 개체 정의(`web/lib/domain/technologySpecimens.ts`)의 `paths`는 이보다 넓은 연결을 미리 적어 둔다. 예를 들어 테스트·CI/CD는 프론트엔드·백엔드·DevOps, Cloud·IaC·Kubernetes는 백엔드·DevOps·데이터 엔지니어와 연결되어 있다. 지도에 지역을 추가할 때 이 목록을 기준으로 삼는다.
...
- 현재 모든 1차 직업의 `completionCaptureIds`가 비어 있으므로, **전문 챕터가 출시되기 전까지 첫 직업 선택이 유지된다.** 두 번째 1차 직업과 2차 전직은 콘텐츠 출시 후에 실제로 열린다.
...
브라우저 저장은 `localStorage`의 `codigdex:save:v3` 한 슬롯에 두며, 변경이 잦은 화면 상태를 한 객체에 평평하게 쌓지 않고 `progress`, `player`, `ui`로 나눈다. 스키마와 마이그레이션은 `web/lib/phaser/save/schema.ts`에 있다.
