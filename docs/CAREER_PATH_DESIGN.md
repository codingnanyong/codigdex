...
직업별 월페이퍼에는 이동 경로와 거점이 그려져 있고, 게임 UI는 번호 원판 대신 각 건물·랜드마크 경계에 맞춘 다각형 포커스와 기술명을 표시한다. 포커스 영역 전체가 클릭 대상이며 호버 시 윤곽과 반투명 채움이 밝아진다. 따라서 새 챕터를 추가할 때는 상위 Path를 다시 그리는 대신 상세 지도 데이터에 목적지의 포커스 영역을 추가하고, 그 목적지를 챕터 지역과 연결한다.
