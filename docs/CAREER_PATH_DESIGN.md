...
전직 전 공통 과정은 버그 연구원 루피가 안내한다. 전직 후 상세 지도에서는 직업별 선배 NPC가 안내를 이어받는다. 플레이어 직업 초상과 안내 NPC는 `assets/characters/career-path/<직업-id>/` 아래의 `player-v1.png`, `guide-v1.png`로 함께 관리한다. 직업을 바꾸면 저장된 1차 직업과 함께 지도·NPC가 즉시 교체된다.
