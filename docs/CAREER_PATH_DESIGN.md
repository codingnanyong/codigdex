...
직업별 월페이퍼에는 이동 경로와 거점이 그려져 있고, 게임 UI는 번호 원판 대신 각 건물·섬의 실제 경계에 맞춘 보이지 않는 클릭 영역과 기술명을 표시한다. 해당 구역에 마우스를 올리면 월페이퍼에서 구역 경계대로 잘라 만든 투명 텍스처만 위로 띄우고 아래에 그림자를 더한다. 사각형이나 다각형 윤곽을 직접 표시하지 않으며 원본 월페이퍼 전체는 움직이지 않는다. 따라서 새 챕터를 추가할 때는 상위 Path를 다시 그리는 대신 상세 지도 데이터에 목적지의 포커스 영역을 추가하고, 그 목적지를 챕터 지역과 연결한다.

전직 전 공통 과정은 버그 연구원 루피가 안내한다. 전직 후 상세 지도에서는 직업별 선배 NPC가 안내를 이어받는다. 플레이어 직업 전신과 안내 NPC는 `assets/characters/career-path/<직업-id>/` 아래의 `player-v2.png`, `guide-v1.png`로 함께 관리한다. 전체 1차 직업 캐릭터 비교표는 `assets/characters/career-path/career-character-guide-v1.png`에 둔다. 직업을 바꾸면 저장된 1차 직업과 함께 지도·NPC가 즉시 교체된다.
...
Path 계층은 화면 왼쪽부터 **전직 전 주니어 개발자 → 1차 직업 → 2차 직업 → 3차 직업**의 네 열로 표시한다. 주니어 개발자 카드에서 모든 1차 직업으로 선이 갈라지고, 각 1차 직업은 조합 가능한 2차 직업으로 이어진다. 완료한 1차 직업 집합과 각 2차 직업의 `requires` 조합을 비교해 두 경로가 모두 끝난 경우에만 이름과 선택 기능을 공개한다. 예를 들어 프론트엔드와 백엔드를 모두 완료하면 풀스택 엔지니어가 열리지만, DevOps까지 완료하지 않았다면 플랫폼 엔지니어/SRE는 계속 잠긴다. 각 2차 직업은 대응하는 3차 직업 하나와 수평으로 연결해 전체 계보에서도 관계가 엉키지 않게 한다.
...
아직 마스터 챕터가 출시되지 않은 2차 직업은 완료 조건을 빈 배열로 유지한다. 이 상태는 3차 전직을 완료한 것으로 간주하지 않으며, 3차 카드는 `???` 미리보기로 남는다. 1차 전문 챕터도 같은 원칙을 쓰되, 완료 조건이 아직 없는 미출시 Preview Path는 플레이어가 다른 1차 직업을 살펴볼 수 있도록 이동만 허용하고 2차 전직의 완료 조합에는 포함하지 않는다.
...
현재 상세 지도 구역과 준비된 대표 기술 개체를 정확히 대조한 결과다. 공유 기술 개체가 다른 지도에 존재하더라도 구역 주제와 일치하지 않으면 준비 완료로 세지 않는다.
...
현재 기술 구역 28개는 모두 일치하는 대표 개체를 갖는다. ch03~ch15의 빠져 있던 Lv.1·Lv.2·Lv.4를 보강했고, React부터 BI 도구까지 새 기술군도 표본과 Lv.1~Lv.5 진화 세트를 준비했다. 전체 준비 현황은 `web/public/assets/monsters/all-monster-evolution-guide-v1.png`에서 한눈에 확인할 수 있다. 튜토리얼의 루프 버그는 진화하지 않는 단일 단계 개체이므로 나머지 칸을 `N/A`로 표시한다.
