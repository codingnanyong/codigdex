...
현재 기술 구역 28개는 모두 일치하는 대표 개체를 갖는다. 새로 보강한 React, 서버 프레임워크, 데이터 파이프라인, 워크플로 오케스트레이션, 기초 통계, 데이터 시각화, BI 도구는 표본과 Lv.1~Lv.5 진화 세트까지 준비했다. 전체 준비 현황은 `web/public/assets/monsters/all-monster-evolution-guide-v1.png`에서 한눈에 확인할 수 있다. 기존 전문 기술 ch03~ch15는 여전히 대부분 Lv.1·Lv.2·Lv.4 제작이 남아 있으며, 현황 이미지에서 `NOT READY`로 표시한다.
